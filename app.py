...
def handle_join_event(user_id, event_id):
    ...
    # Check if the user already joined or if the event is full
    for event in events:
        if event["id"] == event_id:
            if event_id in user_events:
                ...
                # Insert user-event relationship
...
def render_map(events, user_id):
    ...
    for event in events:
        ...
        <div style="font-family:Arial; width:250px;">
            ...
            <p><b>Participants:</b> {event['participants']} / {event['max_participants']}</p>
        </div>
        """
        folium.Marker(
...
# Display Events and Join Buttons
...
# Display All Events with Join Buttons
